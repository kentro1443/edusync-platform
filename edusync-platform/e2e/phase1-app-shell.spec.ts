...
let database: Client;
const email = "admin.minhkhai@edusync.local";
const password = "EduSync-Shell-Test-2026!";
const seededPassword = "EduSync-Demo-2026!";
...
test.describe("App shell Phase 1", () => {
  test("desktop shell exposes real navigation, breadcrumb and keyboard search", async ({ page }) => {
    ...
    await search.click();
    await expect(page.getByRole("dialog", { name: "Tìm kiếm trong EduSync" })).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(page.getByRole("dialog", { name: "Tìm kiếm trong EduSync" })).toHaveCount(0);
