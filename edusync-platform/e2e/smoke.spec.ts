...
test.describe("Kiểm tra khói ứng dụng", () => {
  test("trang chủ hiển thị đúng tiêu đề và điều hướng", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle(/EduSync/);
    await expect(
  ...
  test("tài khoản học sinh demo đăng nhập vào bảng điều khiển", async ({ page }) => {
    ...
    await page.locator("#email").fill("student.minhkhai@edusync.local");
    await page.locator("#password").fill("EduSync-Demo-2026!");
  ...
  test("credential demo của quản trị nền tảng đăng nhập được", async ({ page }) => {
    await page.goto("/login");
    await page.locator("#email").fill("platform@edusync.local");
    await page.locator("#password").fill("EduSync-Demo-2026!");
    await page.getByRole("button", { name: "Đăng nhập" }).click();
