...
test.describe("Phase 1 — thương hiệu và marketing shell", () => {
  test("chuẩn hóa thương hiệu EduSync và cung cấp đầy đủ điều hướng", async ({ page }) => {
    await page.goto("/");

    await expect(page).toHaveTitle(/EduSync/);
    await expect(page.getByRole("banner").getByRole("link", { name: "EduSync - Trang chủ" })).toBeVisible();
