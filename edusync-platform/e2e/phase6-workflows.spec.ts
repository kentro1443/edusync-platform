...
const databaseUrl = process.env.DATABASE_URL ?? "postgresql://edusync:edusync_local@localhost:5432/edusync?schema=public";
...
test.describe("Phase 6 seeded no-code workflows", () => {
  ...
  test("request-changes returns the submission to the owner, who edits and resubmits for review", async ({ page, browser }) => {
    ...
    try {
      ...
      await login(reviewerPage, "teacher.minhkhai@edusync.local");
