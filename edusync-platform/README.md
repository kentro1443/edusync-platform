...
EduSync is a multi-tenant school platform for Vietnamese specialised high schools, built with Next.js, TypeScript, Prisma, PostgreSQL, and Redis. Its core is a **peer-mentor marketplace** where junior students post learning needs and verified senior-student mentors (SAT/IELTS/du học specialties) bid with a price, plus a shared **past-exam/resource library** and **no-code approval workflows** that digitise the paper CLB/event/facility forms (4 signatures, 1–2 weeks → 24–48h). It also includes identity and school administration, staff mentoring/counseling, scheduling, clubs/events, collaboration, notifications, reporting, search, and audit — all permission-aware and tenant-isolated.
...
`AUTH_SECRET`, `DATABASE_URL`, `REDIS_URL`, and `EMAIL_OUTBOX_ROOT`. Managed
Redis URLs may use either `redis://` or the TLS form `rediss://`.

Uploaded documents are stored in a **private Vercel Blob** store. This keeps
PDFs durable across serverless invocations while preserving EduSync's existing
authorization checks: browsers never receive a public Blob URL; authenticated
download and preview routes stream the private object after checking the
current school context and permission.

Create and connect the private store from the linked project directory:

```bash
vercel blob create-store edusync-files --access private
vercel env pull .env.local --yes
```

Vercel adds `BLOB_READ_WRITE_TOKEN` automatically when the store is connected.
Newer OIDC-enabled projects may provide `BLOB_STORE_ID` instead. Production
startup intentionally fails when neither value exists, preventing uploads from
silently falling back to Vercel's ephemeral filesystem. `FILE_STORAGE_ROOT`
remains the local-development adapter only.

`EMAIL_OUTBOX_ROOT` is also a local-development adapter and is not durable on
Vercel. Before enabling transactional email in production, connect a managed
email provider and run scheduled workers through authenticated HTTP endpoints.

The `dev@edusync.local` account is restricted to development and test by
...
| Scope | Role | Email |
| --- | --- | --- |
| Platform | Platform super admin | `platform@edusync.local` |
| Minh Khai | School admin | `admin.minhkhai@edusync.local` |
| Minh Khai | Teacher/staff | `teacher.minhkhai@edusync.local` |
| Minh Khai | Mentor/counselor and teacher/staff | `mentor.minhkhai@edusync.local` |
| Minh Khai | Student | `student.minhkhai@edusync.local` |
| Minh Khai | Parent/guardian | `parent.minhkhai@edusync.local` |
| Minh Khai | Club leader | `club.minhkhai@edusync.local` |
| Minh Khai | Approver/reviewer | `approver.minhkhai@edusync.local` |
| Nguyễn Du | School admin | `admin.nguyendu@edusync.local` |
| Nguyễn Du | Teacher/staff | `teacher.nguyendu@edusync.local` |
| Nguyễn Du | Mentor/counselor | `mentor.nguyendu@edusync.local` |
| Nguyễn Du | Student | `student.nguyendu@edusync.local` |
| Nguyễn Du | Parent/guardian | `parent.nguyendu@edusync.local` |
| Nguyễn Du | Club leader | `club.nguyendu@edusync.local` |
| Nguyễn Du | Approver/reviewer | `approver.nguyendu@edusync.local` |
| Minh Khai + Nguyễn Du | Multi-school admin (school switching) | `admin.multischool@edusync.local` |
| Developer operator | Demo account switcher | `dev@edusync.local` |
...
security find-generic-password \
  -a dev@edusync.local \
  -s edusync-platform-production-dev \
  -w
```
...
Run the following commands one by one from the repository root:

1. Start PostgreSQL and Redis:

```bash
npm --prefix edusync-platform run services:up
```

2. Apply database migrations:

```bash
npm --prefix edusync-platform run db:migrate:deploy
```

3. Load the demo accounts for initial setup:

```bash
npm --prefix edusync-platform run db:seed
```
...
```bash
npm --prefix edusync-platform run dev
```

5. Open <http://localhost:3000> and sign in with a demo account listed above.

After each completed phase or bug fix, the EduSync verification skill runs the automated checks, starts or reuses these services, confirms the website responds at `http://localhost:3000`, and leaves it running for manual testing.
...
- `platform@edusync.local`: tenant directory and platform health counts.
- `admin.minhkhai@edusync.local`: members/settings, all school modules, reports and audit.
- `mentor.minhkhai@edusync.local`: mentor directory, availability, appointments and privacy-projected cases.
- `approver.minhkhai@edusync.local`: workflow review queue and decisions.
- `club.minhkhai@edusync.local`: club roster and event operations.
- `student.minhkhai@edusync.local`: published resources, own bookings/submissions and visible clubs.
- `parent.minhkhai@edusync.local`: linked-student scope only.
- `admin.multischool@edusync.local`: validates explicit school switching and tenant isolation.
