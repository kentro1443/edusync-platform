...
export function renderOutboxEmail(input: Readonly<{
  ...
  if (input.templateKey === "PASSWORD_RESET") {
    ...
    return {
      outboxId: input.id,
      to: input.toAddress,
      subject: "Đặt lại mật khẩu EduSync",
  ...
  if (input.templateKey === "SCHOOL_INVITATION") {
    ...
    return {
      outboxId: input.id,
      to: input.toAddress,
      subject: `Lời mời tham gia ${schoolName} trên EduSync`,
  ...
  if (input.templateKey === "MESSAGE_NOTIFICATION") {
    ...
    return {
      ...
      subject:
        kind === "MENTION"
          ? `${senderName} đã nhắc đến bạn trên EduSync`
          : `Tin nhắn mới từ ${senderName} trên EduSync`,
      text: `${preview}\n\nMở cuộc trò chuyện:\n${href}`,
    };
  }
  return {
    outboxId: input.id,
    to: input.toAddress,
    subject: "Cập nhật từ EduSync",
    text: stringValue(payload, "message", "Bạn có một cập nhật mới trên EduSync."),
  };
}
