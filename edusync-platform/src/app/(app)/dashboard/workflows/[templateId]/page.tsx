    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
      <Card>
        ...
        <h3 className="mt-8 text-base font-bold">Luồng phê duyệt</h3><ol className="mt-3 space-y-3">{draft?.steps.map((step, index) => {
          ...
          return <li key={step.id} className="flex items-start gap-3 rounded-[var(--radius-md)] bg-[var(--color-ink-50)] p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[var(--color-brand-100)] text-sm font-bold text-[var(--color-brand-800)]">{index + 1}</span><div><p className="font-semibold">{step.name}</p><p className="text-xs text-[var(--color-ink-500)]">Vai trò: {step.role}{step.parallelGroup ? ` · nhóm song song ${step.parallelGroup}` : ""}{step.deadlineHours ? ` · hạn ${step.deadlineHours}h` : ""}</p>{condition.field ? <p className="mt-1 text-xs text-[var(--color-brand-700)]">Chỉ chạy khi {condition.field} {condition.operator} {String(condition.value ?? "")}</p> : null}</div></li>;
        })}</ol>

        <h3 className="mt-8 text-base font-bold">Xem trước biểu mẫu</h3>
        <p className="mt-1 text-xs text-[var(--color-ink-500)]">Người nộp sẽ thấy các trường theo đúng thứ tự này.</p>
        <div className="mt-4 space-y-3 rounded-[var(--radius-md)] border border-dashed border-[var(--color-ink-300)] p-4">
          {draft?.fields.length ? draft.fields.map((field) => (
            <div key={field.id}>
              <label className="block text-sm font-semibold text-[var(--color-ink-800)]">
                {field.label}{field.required ? <span className="text-[var(--color-danger-600)]"> *</span> : null}
              </label>
              {field.type === "TEXTAREA" ? (
                <textarea disabled rows={2} className="mt-1 w-full rounded-[var(--radius-md)] border border-[var(--color-ink-200)] bg-[var(--color-ink-50)] px-3 py-2 text-sm text-[var(--color-ink-400)]" placeholder="Văn bản dài…" />
              ) : field.type === "CHECKBOX" ? (
                <div className="mt-1 flex items-center gap-2"><input type="checkbox" disabled /><span className="text-sm text-[var(--color-ink-400)]">Đúng/Sai</span></div>
              ) : field.type === "FILE" ? (
                <div className="mt-1 rounded-[var(--radius-md)] border border-[var(--color-ink-200)] bg-[var(--color-ink-50)] px-3 py-2 text-sm text-[var(--color-ink-400)]">Chọn tệp…</div>
              ) : (
                <input disabled type={field.type === "NUMBER" ? "number" : field.type === "DATE" ? "date" : "text"} className="mt-1 w-full rounded-[var(--radius-md)] border border-[var(--color-ink-200)] bg-[var(--color-ink-50)] px-3 py-2 text-sm text-[var(--color-ink-400)]" placeholder={field.type === "SELECT" ? "Chọn một giá trị…" : ""} />
              )}
            </div>
          )) : <p className="text-sm text-[var(--color-ink-500)]">Chưa có trường nào — thêm trường ở bên phải để xem trước.</p>}
        </div>
      </Card>
      {canEdit ? <div className="space-y-6">
        <Card><h2 className="text-base font-bold">Thêm trường</h2><form action={addWorkflowFieldAction} className="mt-4 space-y-3"><input type="hidden" name="templateId" value={template.id} /><Field id="key" label="Mã trường" required><Input id="key" name="key" required placeholder="budget" /></Field><Field id="label" label="Nhãn hiển thị" required><Input id="label" name="label" required placeholder="Ngân sách dự kiến" /></Field><Field id="type" label="Kiểu (bảng chọn trường)"><Select id="type" name="type" defaultValue="TEXT"><option value="TEXT">Văn bản</option><option value="TEXTAREA">Văn bản dài</option><option value="NUMBER">Số</option><option value="DATE">Ngày</option><option value="SELECT">Lựa chọn</option><option value="CHECKBOX">Đúng/Sai</option><option value="FILE">Tệp đính kèm</option></Select></Field><label className="flex items-center gap-2 text-sm"><input type="checkbox" name="required" /> Bắt buộc</label><Button type="submit" size="sm">Thêm trường</Button></form></Card>
        <Card><h2 className="text-base font-bold">Thêm bước duyệt</h2><p className="mt-1 text-xs leading-5 text-[var(--color-ink-500)]">Bỏ trống điều kiện để bước luôn chạy. Các bước cùng số nhóm sẽ được mở song song.</p><form action={addWorkflowStepAction} className="mt-4 space-y-3"><input type="hidden" name="templateId" value={template.id} /><Field id="stepName" label="Tên bước" required><Input id="stepName" name="name" required placeholder="Duyệt ngân sách" /></Field><Field id="role" label="Vai trò"><Select id="role" name="role" defaultValue="SCHOOL_ADMIN"><option value="SCHOOL_ADMIN">Quản trị trường</option><option value="TEACHER_STAFF">Giáo viên/nhân sự</option><option value="MENTOR_COUNSELOR">Cố vấn</option><option value="APPROVER_REVIEWER">Người duyệt</option></Select></Field><Field id="parallelGroup" label="Nhóm duyệt song song"><Input id="parallelGroup" name="parallelGroup" type="number" min="1" placeholder="Ví dụ: 1" /></Field><Field id="deadlineHours" label="Hạn xử lý (giờ)" description="Quá hạn sẽ tự động được đánh dấu để nhắc việc."><Input id="deadlineHours" name="deadlineHours" type="number" min="1" max="8760" placeholder="Ví dụ: 48" /></Field><div className="rounded-[var(--radius-md)] border border-[var(--color-ink-200)] p-3"><p className="mb-3 text-xs font-bold uppercase tracking-wide text-[var(--color-ink-500)]">Điều kiện tùy chọn</p><div className="space-y-3"><Field id="conditionField" label="Mã trường điều kiện"><Input id="conditionField" name="conditionField" placeholder="budget" /></Field><Field id="conditionOperator" label="Phép so sánh"><Select id="conditionOperator" name="conditionOperator" defaultValue="equals"><option value="equals">Bằng</option><option value="notEquals">Khác</option><option value="truthy">Có / đúng</option><option value="falsy">Không / sai</option></Select></Field><Field id="conditionValue" label="Giá trị"><Input id="conditionValue" name="conditionValue" placeholder="1000000" /></Field></div></div><Button type="submit" size="sm">Thêm bước</Button></form></Card>
