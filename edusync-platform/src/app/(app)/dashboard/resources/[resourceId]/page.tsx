...
export default async function ResourceDetailPage({
  ...
  const currentFile = resource.currentVersion
    ...
    : undefined;
  const previewHref = resource.currentVersion
    ? `/dashboard/resources/${resource.id}/preview?versionId=${resource.currentVersion.id}`
    : undefined;
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="space-y-6">
          <Card>
            ...
            {currentFile?.mimeType === "application/pdf" && previewHref ? (
              <div className="mt-6 overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-ink-200)] bg-[var(--color-ink-50)]">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-ink-200)] px-4 py-3">
                  <div>
                    <h2 className="text-sm font-semibold text-[var(--color-ink-900)]">Xem trước PDF</h2>
                    <p className="text-xs text-[var(--color-ink-500)]">{currentFile.originalName}</p>
                  </div>
                  <Link
                    href={previewHref}
                    target="_blank"
                ...
                <iframe
                  title={`Xem trước PDF ${currentFile.originalName}`}
                  src={previewHref}
                  loading="lazy"
            ...
            {resource.currentVersion && currentFile ? (
