# EduSync Architecture Decision Record
...
EduSync is a Vietnamese-only, local production SaaS for multiple schools. The system needs durable workflows, strict tenant isolation, role-based authorization, local infrastructure, real-time collaboration, secure file access, and a visual marketing site within one Next.js codebase.
...
EduSync is a modular monolith with a separate real-time process:
