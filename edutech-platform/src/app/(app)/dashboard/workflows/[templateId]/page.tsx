  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
      <Card>
        ...
        <h3 className="mt-8 text-base font-bold">Luồng phê duyệt</h3><ol className="mt-3 space-y-3">{draft?.steps.map((step, index) => <li key={step.id} className="flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--color-ink-50)] p-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--color-brand-100)] text-sm font-bold text-[var(--color-brand-800)]">{index + 1}</span><div><p className="font-semibold">{step.name}</p><p className="text-xs text-[var(--color-ink-500)]">Vai trò: {step.role}</p></div></li>)}</ol>
      </Card>
